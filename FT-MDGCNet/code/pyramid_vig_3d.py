...
from gcn_lib_3d import Grapher, act_layer, Grapher_3d_1,Grapher_3d_2,Grapher_3d_3,Grapher_3d_4
    ...
    def __init__(self, opt):
        ...
        for i in range(len(blocks)):
            ...
            for j in range(blocks[i]):
                if i == 0:
                    self.backbone += [
                        Seq(Grapher(channels[i], num_knn[idx], min(idx // 4 + 1, max_dilation), conv, act, norm,
                                    bias, stochastic, epsilon, reduce_ratios[i], n=HW,H=int(HW**0.5), W=int(HW**0.5), drop_path=dpr[idx],
                                    relative_pos=True),
                          FFN(channels[i], channels[i] * 4, act=act, drop_path=dpr[idx])
                         )]
                if i == 1:
                    self.backbone += [
                        Seq(Grapher_3d_2(channels[i], num_knn[idx], min(idx // 4 + 1, max_dilation), conv, act, norm,
                                    bias, stochastic, epsilon, reduce_ratios[i], n=HW,H=int(HW**0.5), W=int(HW**0.5), drop_path=dpr[idx],
                                    relative_pos=True),
                          FFN(channels[i], channels[i] * 4, act=act, drop_path=dpr[idx])
                         )]
                ...
                # if int(HW**0.5) % 4 ==0:
                #     self.backbone += [
                #         Seq(Grapher_3d(channels[i], num_knn[idx], min(idx // 4 + 1, max_dilation), conv, act, norm,
                #                     bias, stochastic, epsilon, reduce_ratios[i], n=HW,H=int(HW**0.5), W=int(HW**0.5), drop_path=dpr[idx],
                #                     relative_pos=True),
                #           FFN(channels[i], channels[i] * 4, act=act, drop_path=dpr[idx])
                #          )]
                ...
                #                     relative_pos=True),
                #           FFN(channels[i], channels[i] * 4, act=act, drop_path=dpr[idx])
                #          )]                         ######Grapher_3d在一二层时，这里改为Grapher,且relativ_pos改为True;在所有层时，这里改为Grapher_3d,且relative_pos改为False
                idx += 1
...
def pvig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=1000, drop_path_rate=0.0, **kwargs):
            ...
            self.blocks = [2,2,6,2] # number of basic blocks in the backbone  ###另一个是【1，1，6，2】
