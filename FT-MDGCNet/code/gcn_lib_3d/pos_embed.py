...
# --------------------------------------------------------
def get_2d_relative_pos_embed(embed_dim, H,W):
    """
...
def get_2d_sincos_pos_embed(embed_dim, H, W, cls_token=False):
    ...
    grid = np.meshgrid(grid_w, grid_h)  # here w goes first  ###其中grid[0]形状为(h,w),grid[1]为(h,w),np.meshgrid功能为生成网格坐标
    grid = np.stack(grid, axis=0)   ### (2,h,w)

    grid = grid.reshape([2, 1, H, W])   ###(2,1,h,w),本程序默认h=w
...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    pos = pos.reshape(-1)  # (M,)
    out = np.einsum('m,d->md', pos, omega)  # (M, D/2), outer product，np.einsum求向量外积
